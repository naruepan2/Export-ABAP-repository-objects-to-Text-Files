...
CONSTANTS c_r3_fugx   TYPE trobjtype  VALUE 'FUGX'.         "#EC NOTEXT
...
*--------------------------------------------------------------------*
"Object header
TYPES:  BEGIN OF ty_s_object_header,
          name  TYPE sobj_name,
          type  TYPE trobjtype,
        END OF ty_s_object_header.
TYPES ty_t_object_header TYPE STANDARD TABLE OF ty_s_object_header.
"Object
TYPES:  BEGIN OF ty_s_object.
        INCLUDE TYPE ty_s_object_header.
TYPES:    pack  TYPE devclass,
        END OF ty_s_object.
...
CLASS lcl_export_prog DEFINITION INHERITING FROM lcl_export_to_text FINAL.
  PUBLIC SECTION.
    "This method is redefined because you can have multiple files with some success and some faults
    "You want to save the success and not cancel all files if one has a fault.
    METHODS export_to_text REDEFINITION.
...
CLASS lcl_export_prog IMPLEMENTATION.
  METHOD export_to_text.
    ...
    TRY.
        "Get source
        get_source( CHANGING ct_export_status = lt_export_status ).

        IF mt_source IS NOT INITIAL.
          "Write to text file
          write_to_file( CHANGING ct_export_status = ct_export_status ).
        ENDIF.
      CATCH lcx_export_to_text.                         "#EC NO_HANDLER
    ENDTRY.
    APPEND LINES OF lt_export_status TO ct_export_status.
  ENDMETHOD.                    "export_to_text
  ...
  METHOD get_source.
    ...
    LOOP AT mt_source INTO ls_source.
      ...
      IF sy-subrc <> 0.
        ...
        ls_export_status-status = icon_led_red.
        ls_export_status-type   = mv_type.
        ls_export_status-name   = lv_program.
        ls_export_status-path   = mv_path.
        MESSAGE e077(ds) WITH '' INTO ls_export_status-message.
        APPEND ls_export_status TO ct_export_status.
        DELETE mt_source WHERE name = ls_source-name.
      ENDIF.
  ...
  METHOD get_source.
    ...
    TRY.
        ...
        lo_component = cl_wdy_md_component=>get_object_by_key(  name    = lv_component_name
                                                                version = if_wdy_md_object=>co_version_active ).
...
CLASS lcl_main_ctr IMPLEMENTATION.
  ...
  METHOD get_object_from_tran.
    ...
    DATA lt_object_hdr  TYPE ty_t_object_header.
    DATA lt_object      TYPE ty_t_object.
    ...
    SELECT obj_name
           object   INTO  TABLE lt_object_hdr
                    FROM  e071
    ...
    "Set object header table to object table
    APPEND LINES OF lt_object_hdr TO lt_object.
  ...
  METHOD get_object_from_func.
    ...
    IF it_r_funcname IS NOT INITIAL.
      SELECT pname  INTO  TABLE lt_program            "#EC CI_SGLSELECT
                    FROM  tfdir
                    WHERE funcname IN it_r_funcname.
    ENDIF.
    IF it_funcname IS NOT INITIAL.
      SELECT pname  INTO  TABLE lt_program            "#EC CI_SGLSELECT
                    FROM  tfdir
                    FOR   ALL ENTRIES IN it_funcname
                    WHERE funcname = it_funcname-table_line.
    ENDIF.

    "Set program table to object table
    APPEND LINES OF lt_program TO lt_object.
        ...
        WHEN c_r3_prog.
          ...
          CALL FUNCTION 'RS_GET_MAINPROGRAMS'
            EXPORTING
              name         = ls_object-name
            TABLES
              mainprograms = lt_program
            EXCEPTIONS
              OTHERS       = 0.

          LOOP AT lt_program INTO lv_program.
            ls_selection-type = ls_object-type.
            ls_selection-name = lv_program.
            CONCATENATE lv_droot '\' p_dprog '\' lv_program INTO ls_selection-path.
            "Check if main program is a function group
            IF  strlen( lv_program ) > 4
            AND lv_program(4) = 'SAPL'.
              "Remove SAPL
              lv_program = lv_program+4.
              ls_selection-type = c_r3_fugr.
              CONCATENATE lv_droot '\' p_dfugr '\' lv_program INTO ls_selection-path.
            ENDIF.
          ENDLOOP.
